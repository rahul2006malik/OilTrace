...
def lee_filter(image_db: np.ndarray, window_size: int = 7, num_looks: float = 1.0) -> np.ndarray:
    """Lee filter for multiplicative SAR speckle noise — literature-standard
    preprocessing (Lee Sigma filter, typically 7x7, shows up in nearly every
    serious SAR oil-spill detection pipeline: e.g. Al-Ruzouq et al.'s
    conventional pipeline, the Suez Canal detection study, the self-evolving
    Sentinel-1 study — all specifically cite a 7x7 Lee/Lee-Sigma filter).

    CRITICAL DETAIL multiple papers are explicit about: the Lee filter's
    multiplicative noise model is only valid in LINEAR intensity space, not
    in dB (log) space. Our data ships as Sigma0 dB (confirmed via
    inspection — negative values, e.g. -54 to -12). This function converts
    dB -> linear, filters, then converts back to dB, rather than filtering
    the dB values directly (which several of these same papers note is a
    common mistake — one explicitly lists "linear transformation from dB
    to actual luminosity values prior to segmentation" as a required step).

    Operates per-channel independently (VV, VH are different physical
    quantities and should not share filter statistics).

    num_looks: for Sentinel-1 GRD, this varies by product/mode — 1.0 is a
    conservative default (assumes single-look, i.e. MORE noise, so the
    filter is more conservative/gentle) rather than guessing a higher
    look count that would over-smooth if wrong.
    """
    try:
        from scipy.ndimage import uniform_filter
    except ImportError:
        raise ImportError("lee_filter requires scipy (pip install scipy — already present on Kaggle's base image).")

    if image_db.ndim != 3:
        raise ValueError(f"Expected (H, W, C), got shape {image_db.shape}")

    out = np.empty_like(image_db, dtype=np.float32)
    for c in range(image_db.shape[2]):
        channel_db = image_db[:, :, c].astype(np.float64)
        linear = 10.0 ** (channel_db / 10.0)  # dB -> linear intensity

        mean = uniform_filter(linear, size=window_size)
        mean_sq = uniform_filter(linear * linear, size=window_size)
        variance = np.maximum(mean_sq - mean * mean, 0.0)

        cu2 = 1.0 / num_looks
        ci2 = variance / np.maximum(mean * mean, 1e-12)
        weight = np.where(ci2 > cu2, 1.0 - (cu2 / np.maximum(ci2, 1e-12)), 0.0)
        weight = np.clip(weight, 0.0, 1.0)

        filtered_linear = mean + weight * (linear - mean)
        filtered_linear = np.maximum(filtered_linear, 1e-12)  # guard log(0)
        out[:, :, c] = (10.0 * np.log10(filtered_linear)).astype(np.float32)  # linear -> dB
...
class _SceneCache:
    ...
    def get_or_load(self, sample_id: str, image_path: str, mask_path: str):
        ...
        if self.despeckle:
            # Must happen BEFORE percentile_normalize — lee_filter needs
            # real dB values to convert to linear correctly; running it
            # after normalization would filter an already-rescaled [0,1]
            # signal with the wrong noise model entirely.
...
class SARTileDataset(Dataset):
    ...
    random_crop (train only — leave False for validation): instead of
    always cropping at the tile_ref's precomputed (row, col), samples a
    fresh random valid crop position every time __getitem__ is called.

    WHY THIS EXISTS: with tile_size=512 and stride=384, build_tile_index
    precomputes exactly 25 fixed crop windows per 2048x2048 image. Every
    epoch, every sample_id sees the literal same 25 windows (only
    flipped/rotated by _augment) — not 25 independent views, the same 25
    views repeated. Across two separate full runs (with and without an
    encoder freeze), train_loss collapsed toward zero while val_oil_IoU
    declined from epoch 1 onward — the textbook signature of memorizing
    fixed crop specifics (exact speckle realization, exact boundary
    pixels at those 25 positions) rather than learning transferable oil
    texture. random_crop=True turns ~2,300 train images into effectively
    unlimited distinct training views instead of a fixed, memorizable
    ~57,000-tile set, without touching validation's deterministic grid at
    all (evaluation numbers stay comparable across runs).
    """

    def __init__(self, tile_refs: list[TileRef], augment: bool = False,
                 cache_size: int = DEFAULT_CACHE_SIZE, scene_size: int | None = None,
                 random_crop: bool = False, oil_biased_crop: bool = False,
                 oil_bias_prob: float = 0.7, despeckle: bool = False):
        self.tile_refs = tile_refs
        self.augment = augment
        self.random_crop = random_crop
        self.oil_biased_crop = oil_biased_crop
        self.oil_bias_prob = oil_bias_prob
        self.scene_size = scene_size
        if random_crop and scene_size is None:
            raise ValueError("random_crop=True requires scene_size to compute valid crop bounds.")
        if oil_biased_crop and not random_crop:
            raise ValueError("oil_biased_crop=True requires random_crop=True (it's a bias on top of random cropping).")
        self._cache = _SceneCache(maxsize=cache_size, scene_size=scene_size, despeckle=despeckle)
    ...
    def __getitem__(self, idx: int):
        ...
        t = TILE_SIZE
        if self.random_crop:
            max_start = self.scene_size - t
            if self.oil_biased_crop:
                # LITERATURE-BACKED: constrained/biased patch selection is
                # reported to substantially raise oil-class IoU in prior
                # SAR oil-spill work (0.86-0.89 vs much lower for
                # unconstrained patches, per Hidalgo et al.'s cited
                # results). With oil_bias_prob probability, center the
                # crop on a randomly chosen oil pixel (with jitter so it's
                # not always dead-center); otherwise (or if this scene has
                # no oil pixels at all) fall back to pure uniform random.
                oil_ys, oil_xs = np.nonzero(mask)
                if len(oil_ys) > 0 and random.random() < self.oil_bias_prob:
                    pick = random.randrange(len(oil_ys))
                    y, x = int(oil_ys[pick]), int(oil_xs[pick])
                    jitter = t // 4
                    r = int(np.clip(y - t // 2 + random.randint(-jitter, jitter), 0, max_start))
                    c = int(np.clip(x - t // 2 + random.randint(-jitter, jitter), 0, max_start))
                else:
                    r = random.randint(0, max_start)
