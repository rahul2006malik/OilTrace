...
# "more filters" lever from Trujillo-Acatitla et al.'s own reported best
# config — default matches smp.Unet's own default (unchanged behavior);
# 'wide' roughly doubles decoder channel counts at every stage.
DECODER_WIDTH_PRESETS = {
    "default": (256, 128, 64, 32, 16),
    "wide": (512, 256, 128, 64, 32),
}
...
class FocalLoss(nn.Module):
    """Standard pixel-wise Focal Loss (Lin et al., arxiv.org/abs/1708.02002),
    opt-in via --loss focal. This is what the dataset's own authors
    (Trujillo-Acatitla et al. 2024) specifically credited as their single
    biggest lever going from a plain U-Net to their best segmentation
    result (96% IoU) — NOT FocalTverskyLoss above, which is a different,
    related-but-distinct loss. Both are available so you can A/B them.

    alpha balances positive/negative class weight (oil pixels are rare);
    gamma down-weights already-easy, correctly-classified pixels so
    training focuses on hard/misclassified ones — exactly the effect
    wanted when ~98% of pixels are trivially-easy background.

    Same fp32-forced-under-autocast pattern as the other losses above,
    for the same reason: log(sigmoid(...)) reductions can NaN under fp16.
    """
...
def compute_false_positive_rate(logits: torch.Tensor, targets: torch.Tensor, threshold: float = 0.5) -> float:
    """NOTE: no longer used by train()'s validation loop — that now computes
    corpus-level false-positive rate directly (see the eval block below),
    since per-batch averaging had the same noise problem as compute_oil_iou
    did. Kept here as a standalone utility (e.g. for ad-hoc analysis).
...
def compute_oil_iou(logits: torch.Tensor, targets: torch.Tensor, threshold: float = 0.5) -> float:
    """NOTE: no longer used by train()'s validation loop — see the
    corpus-level micro-averaging in the eval block below, which replaced
    this per-batch-then-averaged approach after it was found to inflate
    val_oil_IoU by ~0.13 and be noisy enough to pick an undertrained
    checkpoint as "best". Kept here as a standalone utility.
...
def train(args):
    ...
    train_ds = SARTileDataset(train_tiles, augment=True, scene_size=train_scene_size,
                               random_crop=args.random_crop_train,
                               oil_biased_crop=args.oil_biased_crop,
                               despeckle=args.despeckle)
    val_ds = SARTileDataset(val_tiles, augment=False, scene_size=val_scene_size,
                             despeckle=args.despeckle)
    # random_crop/oil_biased_crop are NEVER applied to val_ds — validation
    # must stay on the fixed deterministic grid so val_oil_IoU numbers
    # stay comparable across runs/checkpoints. despeckle DOES apply to
    # both — it's a preprocessing choice about the input signal itself,
    # not a training-time augmentation, so train/val must match or the
    # model would be evaluated on a different input distribution than it
    # was trained on.
    ...
    model = smp.Unet(
        ...
        # Opt-in spatial+channel attention in the decoder (SCSE paper:
        # arxiv.org/abs/1808.08127). None by default = identical to old
        # behavior. Multiple recent SAR oil-spill segmentation papers
        # report attention modules as a real lever on this exact problem
        # (GSCAT-UNET, NOS-Net's RCBA, LRA-UNet's SimAM), but at least one
        # reported user case where 'scse' fit slower and performed worse
        # — treat this as worth A/B testing, not an assumed win.
    ...
    if args.loss == "focal_tversky":
        ...
        print(f"Using FocalLoss (alpha={args.focal_alpha}, gamma={args.focal_gamma}) — "
              f"matches what the dataset's own authors credited as their biggest single lever.")
    else:
    ...
    #
    # Discriminative LR via two param groups: encoder gets
    # lr * encoder_lr_mult, decoder/head get the full lr. With the default
    # mult=1.0 this is mathematically identical to the old single-group
    # AdamW call — only changes behavior if you pass --encoder-lr-mult < 1.
    ...
    # ADDED after seeing intermittent large loss spikes (e.g. loss=3.4365
    # mid-epoch-3 in an otherwise-converging run, right before a sharp
    # drop into near-zero loss everywhere else) — a classic sign of an
    # occasional unstable gradient step. --grad-clip alone caps the size
    # of a bad step but doesn't stop the LR from being too aggressive in
    # the first place. Linear warmup ramps LR up gradually over the first
    # N optimizer steps instead of hitting full LR from step 1, which is
    # a standard, low-risk stabilizer. Off by default (0 = old behavior).
    ...
    # Decide initial freeze state based on where we're ACTUALLY starting —
    # matters for --resume: if resuming past the freeze window, the
    # encoder must already be unfrozen, not frozen-then-never-unfrozen
    # (the in-loop check below only fires on an exact epoch match, which
    # a resumed run starting after that epoch would never hit).
    if args.freeze_encoder_epochs > 0 and start_epoch <= args.freeze_encoder_epochs:
        for p in raw_model.encoder.parameters():
            p.requires_grad = False
        print(f"Encoder frozen for epochs {start_epoch}-{args.freeze_encoder_epochs} "
              f"— only the decoder trains until then.")

    for epoch in range(start_epoch, args.epochs + 1):
        if args.freeze_encoder_epochs > 0 and epoch == args.freeze_encoder_epochs + 1:
            for p in raw_model.encoder.parameters():
                p.requires_grad = True
            print(f"Epoch {epoch}: unfreezing encoder (encoder LR = "
                  f"{args.lr * args.encoder_lr_mult}, decoder LR = {args.lr}).")
        ...
        for batch_idx, (images, masks) in enumerate(train_loader):
            ...
            if args.warmup_steps > 0 and global_step < args.warmup_steps:
                ...
                for g, base in zip(optimizer.param_groups, base_lrs):
                    g["lr"] = base * warmup_factor
            # Once global_step reaches warmup_steps, LR is left exactly at
            # base_lrs and this block simply stops touching it — from then
            # on ReduceLROnPlateau owns the LR entirely. Deliberately NOT
            # resetting to base_lrs every step post-warmup, since that
            # would silently undo any reduction ReduceLROnPlateau already
            # applied.
        ...
        model.eval()
        # CORPUS-LEVEL micro-averaged IoU, not per-batch mean. This was
        # previously applied ONLY as a one-off runtime patch directly on
        # Kaggle and got silently reverted the next time Cell 2 re-copied
        # this file from use-scripts — folding it in here permanently so
        # that can't happen again.
        #
        # WHY per-batch mean was wrong: at batch_size=32, only ~2% of
        # pixels are oil, so most "oil-containing" batches contain just
        # one or two oil tiles. Averaging per-batch IoUs means a huge
        # share of the reported number is really an average of single-
        # tile, high-variance estimates — not a stable corpus-level
        # number. It measurably inflated val_oil_IoU (~0.13 higher on the
        # run this was first caught on) and was noisy enough to make an
        # undertrained early checkpoint look like the best one.
        ...
        mean_fp_rate = fp_pixels_sum / max(fp_pixels_count, 1) if fp_pixels_count > 0 else float("nan")
        fp_note = (f", val_false_positive_rate={mean_fp_rate:.5f} (corpus pixel-level)"
                    if fp_pixels_count > 0 else
        ...
        print(f"=== Epoch {epoch}: train_loss={avg_train_loss:.4f}, "
              f"val_oil_IoU={mean_val_iou:.4f} (corpus micro-averaged)"
              f"{fp_note} ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--freeze-encoder-epochs", type=int, default=0,
                         help="Freeze the pretrained encoder for this many initial epochs "
                              "(decoder-only training), then unfreeze. 0 (default) never "
                              "freezes — same as old behavior. Useful when val performance "
                              "peaks very early and then plateaus/declines while train_loss "
                              "keeps dropping (fast-overfit signature): lets the decoder learn "
                              "to use the pretrained features before both start moving together.")
    parser.add_argument("--encoder-lr-mult", type=float, default=1.0,
                         help="Encoder LR = --lr * this value; decoder/head always use --lr. "
                              "1.0 (default) = same LR for both, identical to old behavior. "
                              "Set < 1 (e.g. 0.1) so the pretrained encoder adapts slower than "
                              "the randomly-initialized decoder once unfrozen.")
    parser.add_argument("--random-crop-train", action="store_true",
                         help="Sample a fresh random crop position per __getitem__ call during "
                              "training instead of the fixed precomputed 25-tile grid (validation "
                              "always stays on the fixed grid regardless of this flag). Off by "
                              "default. Targets the specific memorization pattern seen when "
                              "train_loss collapses while val_oil_IoU declines from epoch 1.")
    parser.add_argument("--oil-biased-crop", action="store_true",
                         help="Requires --random-crop-train. Biases crop position toward "
                              "oil-containing pixels when the source scene has any, instead of "
                              "pure uniform random. Literature-backed (constrained patch "
                              "selection reported substantially higher oil-class IoU vs "
                              "unconstrained patches in prior SAR oil-spill work). Off by default.")
    parser.add_argument("--despeckle", action="store_true",
                         help="Apply a Lee filter (7x7, literature-standard for SAR oil-spill "
                              "preprocessing) to reduce speckle noise, correctly done in linear "
                              "intensity domain then converted back to dB. Applied identically to "
                              "train AND val (it's a preprocessing choice, not an augmentation). "
                              "Off by default — adds real CPU cost per image load, test its "
                              "effect before assuming it's free.")
    parser.add_argument("--decoder-attention", type=str, default="none", choices=["none", "scse"],
                         help="Attention module in the U-Net decoder. 'none' (default) = old "
                              "behavior. 'scse' adds spatial+channel squeeze-excitation attention "
                              "— test via A/B, not a guaranteed win.")
    parser.add_argument("--decoder-width", type=str, default="default", choices=list(DECODER_WIDTH_PRESETS.keys()),
                         help="'default' = smp.Unet's own default decoder channel widths (unchanged "
                              "behavior). 'wide' roughly doubles them at every decoder stage — "
                              "the 'more filters' lever from Trujillo-Acatitla et al.'s own best "
                              "reported config. More GPU memory and slower per epoch.")
    parser.add_argument("--focal-alpha", type=float, default=0.75,
                         help="Positive-class weight for --loss focal. Oil pixels are rare, so "
                              "this is above 0.5 by default to upweight them.")
    parser.add_argument("--focal-gamma", type=float, default=2.0,
                         help="Focusing parameter for --loss focal — higher values down-weight "
                              "easy/already-correct pixels more, concentrating gradient on hard ones.")
    parser.add_argument("--warmup-steps", type=int, default=0,
                         help="Linearly ramp LR from 0 to full value over this many optimizer "
                              "steps at the very start of training. 0 (default) = old behavior, "
                              "full LR from step 1. Targets intermittent large loss spikes seen "
                              "mid-epoch on some runs.")
    ...
    parser.add_argument("--loss", type=str, default="dice_bce", choices=["dice_bce", "focal_tversky", "focal"],
